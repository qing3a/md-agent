...
- **/view 面板渲染层**：iframe 沙箱 + postMessage 桥（视图经宿主调 `/api/*`，仅允许 api 前缀，真机验证通过）：`/view graph` 知识库结构导航（目录层级缩进树 + 项目色点 + 孤立/悬空标记，点击文档右侧显示出链/入链并可跳转——全图环形布局发散已弃用）、`/view board` 任务看板、`/view pending` 审核面板（待审 + 健康审计）、`/view ops` 自动化运营面板、`/view <html>` 渲染 kb 内本地 HTML、`/view off` 或 Esc 关闭；**单视图**（同一时刻只开一个详情页，新开替换旧的，`/view audit`/`/view automation` 兼容映射到合并面板）；分屏参照（header「分屏/全屏」切换：终端占左 40% 保持可见可参照 + 视图右 60%，选择记忆、关闭视图自动回全屏）；沙箱脚本错误上报宿主写终端提示、桥请求 20s 超时兜底；面板写操作（批准待审/改任务/补链/卸载）经桥回传后主界面状态栏即时刷新；关闭视图后焦点自动归还终端输入框（面板往返无需点终端）
- **审核面板**：`/view pending`（兼容 `/view audit`）：上段待审三栏（清单批量勾选/目标上下文+绿色 diff/可编辑内容，支持编辑后批准与批量批准/拒绝），下段健康审计折叠区——补链建议一键 [应用]、悬空/孤立/重复分组展示（`/audit` 终端版保留）
- **自动化运营面板**：`/view ops`（兼容 `/view automation`）：上段自动化控制区（心跳开关/周期/自动补链/巩固器/经验闭环/会话归档说明），下段运营数据区（记忆热度 / token 用量与缓存命中 / 近 7 日检索·工具·待审趋势 / 活动时间线与未读通知，3s 轮询）
