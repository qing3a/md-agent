...
本地常驻托盘后台，所有知识库、会话、业务文件以Markdown纯文本落盘，无黑盒向量存储。原生兼容标准MCP协议，既可作为记忆/工具层为DeepSeek Harness、各类MCP兼容运行时提供本地能力，也支持对接Ollama、其他商用推理端点；同时内置MCP客户端，可主动连接第三方MCP服务与自研业务中台（stdio已实现，HTTP传输规划中）。依托标准MCP协议向外暴露本地检索、图谱、任务能力，聚合外部服务，形成双向MCP生态。

核心设计：所有AI写入内容强制人工审核，知识可读、可追溯、可控。
