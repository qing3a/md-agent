...
- **会话归属项目**：会话 = 项目的内容——左侧栏按项目分组展示全部会话（个人空间 = 默认项目，未新建项目时会话归它）；点击任意项目的会话即自动切到该项目上下文（不存在「切换项目」操作，使用哪个项目的对话就操作哪个项目）
- **新建对话选项目**：点「新对话」弹项目选择器（个人空间 + 全部项目 + 新建入口），选中即在该项目开新会话；侧边栏项目组头点击 = 在该项目开新会话
- **项目制**：每个项目 = `kb/projects/<id>/` 下的独立迷你知识库（自己的 L1 规范、笔记、会话、图谱/活动/任务三库）；项目之间**完全隔离**——检索、会话、记忆、图谱绝不串用（隔离由 root 指向 + 遍历排除双层保证，非约定）；个人空间 = 现有全局 KB，零迁移
- **隔离语义**：项目对话默认只操作/只读取本项目内容（检索、图谱、文件经 `X-Project` 头解析到项目根）；跨项目文件操作显式指定项目即可（不额外授权）
- **新建向导三模板**：空白 / 律师案件（案件总览·证据清单·时间线·法律研究）/ 猎头项目（职位需求·候选人台账·客户·沟通记录）；`/spaces` 列出项目空间
- **新手引导**：首次启动未配置连接时自动弹出三步向导（连接 DeepSeek 预填默认值 → 创建第一个项目 → 开始使用）
